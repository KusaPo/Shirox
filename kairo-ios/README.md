...
- Episode rows show AniList episode previews when available and Jikan/MyAnimeList titles. Missing preview images fall back to the show's artwork; downloaded episodes can also show a still from the middle of the local video when iOS supports frame extraction. Missing external IDs trigger an exact-title metadata lookup. Long series use 100-episode ranges with cached, rate-spaced paginated requests. Episode titles depend on external coverage and may be unavailable.
