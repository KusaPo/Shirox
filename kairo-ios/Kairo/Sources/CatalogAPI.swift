...
actor CatalogAPI {
    ...
    func trending() async throws -> [Anime] {
        ...
        id title { english romaji } coverImage { extraLarge large } bannerImage description episodes
    ...
    func search(_ keyword: String) async throws -> [Anime] {
        ...
        items { id anilistId titleRomaji titleEnglish coverImage bannerImage episodeCount seasonYear genres } } }
        """
    ...
    static func anilistAnime(_ row: [String: Any]) -> Anime? {
        ...
        return Anime(anilistID: id, title: title, alternateTitle: names["romaji"] as? String, cover: image(row["coverImage"]), banner: image(row["bannerImage"]), synopsis: (row["description"] as? String ?? "").replacingOccurrences(of: "<[^>]+>", with: "", options: .regularExpression), episodeCount: row["episodes"] as? Int ?? next.map { max(0, $0 - 1) }, year: row["seasonYear"] as? Int, genres: row["genres"] as? [String] ?? [])
    }
    static func animexAnime(_ row: [String: Any]) -> Anime? {
        ...
        return Anime(anilistID: row["anilistId"] as? Int, sourceID: slug, title: title, alternateTitle: row["titleRomaji"] as? String, cover: image(row["coverImage"]), banner: image(row["bannerImage"]), episodeCount: row["episodeCount"] as? Int, year: row["seasonYear"] as? Int, genres: row["genres"] as? [String] ?? [])
    }
}
